...
> 引入base64.min.js,下边是base64.min.js代码
```
...
```
    console.log(Base64.encode('yangjun杨俊'));
    const b = Base64.encode('yangjun杨俊')
...
```
 operatorCookie: any = {
    ...
    setCookie(name, value, time) {
      ...
      document.cookie = name + '=' + Base64.encode(value) + ';expires=' + exp['toGMTString']();
    },
    ...
    getCookie(name) {
      ...
      if (arr = document.cookie.match(reg)) {
        console.log(Base64.decode(arr[2]));
  ...
  <!-- 使用 -->
  operatorCookie.setCookie('eWFuZ2p1bg', account.value, 'd7')
  operatorCookie.getCookie('eWFuZ2p1bg')
```